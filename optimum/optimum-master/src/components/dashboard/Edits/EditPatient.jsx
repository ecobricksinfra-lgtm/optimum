...
const EditPatient = ({ editId, setEditId, setRefresh }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            img && formData.append("profilePic", img)
            for (let key in data) {
                formData.append(key, data[key])
            }
            const res = await axios.put(
                `http://ec2-52-66-228-175.ap-south-1.compute.amazonaws.com:3000/api/patient/${editId}`,
                formData,
                {
                    headers: {
